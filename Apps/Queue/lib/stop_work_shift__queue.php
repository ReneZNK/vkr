...
function stopWorkShift__queue($_request) 
{
	if ( 
	...
	)
	{
		...
		if ( $actual_time_CHECK[0]['end_time'] == NULL ) 
		{
			...
			// Изменение записи о фактическом прибывании на рабочем месте
			R::exec( 'UPDATE actual_time_at_work SET `end_time` = :end_time WHERE `id` = :id', [':end_time' => date('Y-m-d H:i:s'), ':id' => $LAST_work_shift[0]['id_actual_time_at_work']] );
			echo 'Смена завершена';
			//echo messageBox__Top('<p>Смена завершена</p>');
		}
		else 
		{
			echo 'Ошибка :( Сначала нужно начать смену';
			//echo messageBox__Top('<p>Ошибка :( Сначала нужно начать смену</p>');
		}
	}
	else 
	{
